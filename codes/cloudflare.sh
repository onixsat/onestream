...
WHITE=`tput setaf 4 bold`
echo "${WHITE}Colors"
BLUE=`tput setaf 4 smul`
echo "${BLUE}With tput"
CYAN=`tput setaf 4 blink`
echo "${CYAN}is less"
RED=`tput setab 1 setaf 4`
echo "${RED}verbose!"

}
#function nginx(){
#    if ($host != "ospro.pt") { return 501 '{"code":"3", "message": "Não permitido"}'; }
#    set $access_denied 1;
#    if ($request_uri = /index.php?a=a) {
#        set $access_denied 0;
#    }
#    if ($access_denied) { return 502 '{"code":"3", "message": "Não permitido"}'; }   
#}
...
function esperar3(){
  ...
  while [ "$(ps a | awk '{print $1}' | grep $pid)" ]; do
    ...
    echo -ne "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b"
    sleep $delay
  done
  echo -ne "\b\\r${CHECK_MARK}${WHITE} ${done}"
}
...
function esperar(){
  ...
  low="\033#0"
  printf "\r${RED}[${GREEN}SUCESSO${RED}]${WHITE}%s${NC}\n${NC}" "${CINZA} $done_msg"
}
# --- Functions ---
globais
...
file2="value22"
	...
	step "1"
        try esperar3 "sslcert" "Adionando public " "File public adicionado"
	next
    step "2"
	   try esperar3 "sslkey" "Adionando private " "File private adicionado" 
    next

    esperar "sleep 2" "Aguarde..." "O ssl foi adicionado corretamente!"
    F_VDOBLE="\033[0m\033[2;33m"
    echo -e "\n${CINZA}${F_VDOBLE}0 Certificates located at: /etc/letsencrypt/live//${NC}${NORMAL}"
	echo $file1
	echo $file2
